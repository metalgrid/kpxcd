...
// Package security provides secure memory helpers using runtime/secret
// and unix memory locking primitives.
package security
...
// Do runs f inside a runtime/secret.Do scope when available, preventing
// heap escape of sensitive data. Falls back to a direct call when
// GOEXPERIMENT=runtimesecret is not set.
func Do(f func()) {
	secret.Do(f)
}
...
// Use Alloc/Wipe directly instead of this type for simple cases.
...
// Uses an explicit loop to prevent compiler optimization.
